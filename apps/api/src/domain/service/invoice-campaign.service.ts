...
export class InvoiceCampaignService {
  ...
  async sendInvoiceReminder(
    ...
    try {
      ...
      const uploadUrl = await this.uploadPdfToS3(pdfBuffer)
  ...
  async uploadPdfToS3(pdfBuffer: Buffer) {
    const uploadUrl = await this.s3ClientFactory.uploadObject('invoices', pdfBuffer, {
  ...
  async sendViaWhatsapp(
    ...
    console.log('Sending WhatsApp via Twilio to ', user?.phone)
    ...
    try {
      ...
      console.log('Error sending WhatsApp via Twilio', error)
  ...
  async sendInvoiceViaEmail(
    ...
    if (!buffer) {
      ...
      buffer = await this.s3ClientFactory.getObjectBuffer(uploadUrl, { isPrivateBucket: false })
    }
    if (!jobId) return
  ...
  async createPdfBuffer(invoice: Invoice): Promise<Buffer> {
    const siteImage =
      ...
      (await this.s3ClientFactory.getObjectBuffer(invoice.institution?.logo, {
        isPrivateBucket: false,
      }))
    const parentInvoice = invoice.invoiceParentId
      ? await this.getInvoiceById(invoice.invoiceParentId, invoice.institutionId)
      : null
    const site =
      invoice.site ??
      ...
      try {
        ...
        const drawTableHeader = (yPos: number) => {
          ...
          doc
            ...
            .text('Lessons', 330, headerY + 7, { width: 60, align: 'center' })
        ...
        const leftValueWidth = 135

        doc.fontSize(10).font('NotoSansTC')

        // Invoice Number
        doc.fillColor('#666').text('Invoice number', leftLabelX, 120)
        doc.fillColor('black').text(invoiceNumber, leftValueX, 120, { width: leftValueWidth })

        // Date of issue
        const dateText = dayjs(invoice.createdAt).tz(timeZoneId).format('MMMM D, YYYY')
        doc.fillColor('#666').text('Date of issue', leftLabelX, 138)
        doc.fillColor('black').text(dateText, leftValueX, 138, { width: leftValueWidth })

        // Bill to
        const payByText = invoice.payBy ?? ''
        doc.fillColor('#666').text('Bill to', leftLabelX, 156)
        doc.fillColor('black').text(payByText, leftValueX, 156, { width: leftValueWidth })

        const payByHeight = doc.heightOfString(payByText, { width: leftValueWidth })
        const leftTableEndY = 156 + payByHeight

        // Company Details (Right Table, placed to the right of the first table)
        const rightLabelX = 300
        const rightValueX = 365
        const rightValueWidth = 190

        const companyName = invoice.institution?.name ?? ''
        const addressLines = [
          invoice.institution?.address?.addressLine1,
          invoice.institution?.address?.addressLine2,
          [invoice.institution?.address?.area, invoice.institution?.address?.state]
            .filter(Boolean)
            .join(', '),
          invoice.institution?.address?.country,
        ].filter(Boolean)
        ...
        // Company Name
        if (companyName) {
          doc.fillColor('#666').text('Company', rightLabelX, rightY)
          doc.fillColor('black').text(companyName, rightValueX, rightY, { width: rightValueWidth })
          rightY += Math.max(16, doc.heightOfString(companyName, { width: rightValueWidth })) + 2
        }

        // Address
        if (formattedAddress) {
          doc.fillColor('#666').text('Address', rightLabelX, rightY)
          doc
            .fillColor('black')
            .text(formattedAddress, rightValueX, rightY, { width: rightValueWidth })
          rightY += doc.heightOfString(formattedAddress, { width: rightValueWidth }) + 2
        }

        // Phone
        if (phoneText) {
          doc.fillColor('#666').text('Phone', rightLabelX, rightY)
          doc.fillColor('black').text(phoneText, rightValueX, rightY, { width: rightValueWidth })
          rightY += doc.heightOfString(phoneText, { width: rightValueWidth })
        }

        const rightTableEndY = rightY
        const row1EndY = Math.max(leftTableEndY, rightTableEndY)

        // Second Big Row: Payment Methods (spans entire width)
        let pmY = Math.max(210, row1EndY + 15)

        if (payoutMethods.length > 0) {
          doc.fontSize(9).fillColor('#888').font('NotoSansTC').text('Payment Methods', 40, pmY)
          pmY += 14
          ...
          for (const method of payoutMethods) {
            ...
            const detailLines: Array<{ text: string; isLink?: boolean; linkUrl?: string }> = []

            if (details?.bankName) {
              detailLines.push({ text: `Bank: ${details.bankName}` })
            }
            if (details?.bankBranch) {
              detailLines.push({ text: `Branch: ${details.bankBranch}` })
            }
            if (details?.accountName) {
              detailLines.push({ text: `Name: ${details.accountName}` })
            }
            if (details?.accountId) {
              detailLines.push({ text: `Account: ${details.accountId}` })
            }
            if (details?.payoutUrl) {
              // Add space after URL delimiters to allow clean line wrapping
              const formattedUrl = details.payoutUrl.replace(/([/?=&#_.-])/g, '$1 ')
              detailLines.push({
                text: formattedUrl,
                isLink: true,
                linkUrl: details.payoutUrl,
              })
            }
            if (details?.successMessage) {
              detailLines.push({ text: details.successMessage })
            }
            if (method.description) {
              detailLines.push({ text: method.description })
            }

            doc.fontSize(9).font('NotoSansTC')

            const nameHeight = doc.heightOfString(methodName, {
              width: nameWidth,
            })

            const startMethodY = pmY

            // Render Method Name on the left (x = 40, width = 140)
            doc.fillColor('black').text(methodName, nameX, startMethodY, {
              width: nameWidth,
            })

            // Render Method Details on the right (x = 190, width = 365)
            let currentDetailsY = startMethodY
            for (const line of detailLines) {
              ...
              if (line.isLink && line.linkUrl) {
                doc.fillColor(blue)
                doc.text(line.text, detailsX, currentDetailsY, {
                  ...lineOptions,
                  link: line.linkUrl,
                  underline: true,
                })
                ...
                doc.fillColor('black')
                doc.text(line.text, detailsX, currentDetailsY, lineOptions)
              }
              const lineH = doc.heightOfString(line.text, lineOptions)
              currentDetailsY += lineH + 2
            }

            const detailsHeight = currentDetailsY - startMethodY
            const methodHeight = Math.max(nameHeight, detailsHeight)

            pmY = startMethodY + methodHeight + 6
          }
        }

        // "Pay online" button at the bottom of the final payment method
        const btnX = 40
        const btnY = pmY + 8
        const btnWidth = 100
        const btnHeight = 24

        if (paymentReceiptUploadLink) {
          doc.roundedRect(btnX, btnY, btnWidth, btnHeight, 4).fill(blue)
          doc
            .fontSize(10)
            .font('NotoSansTC')
            .fillColor('#ffffff')
            .text('Pay online', btnX, btnY + 6, {
              width: btnWidth,
              align: 'center',
              link: paymentReceiptUploadLink,
            })
          doc.fillColor('black')
        }

        // Table header - dynamically calculate top position below "Pay online" button
        const payOnlineEndY = paymentReceiptUploadLink ? btnY + btnHeight : pmY
        const tableHeaderTop = Math.max(280, payOnlineEndY + 20)
        ...
        doc
          ...
          .text('Lessons', 330, tableHeaderTop + 7, { width: 60, align: 'center' })
        ...
        for (const enrollCourse of enrollCourses) {
          ...
          for (const enrollInto of enrollCourse.enrollInto) {
            ...
            const amount = rest.price || 0

            // ✅ Calculate unit price as amount divided by quantity
            const unitPrice =
              rest.priceType === PriceType.PER_LESSON ? amount / lessonCount : amount
  ...
  async getAutomationSettings(institutionId: number) {
    let settings = await this.automationSettingsRepository.findByType(
      institutionId,
      AutomationSettingsType.INVOICE_CAMPAIGN_DUPLICATION
    )
    if (!settings) {
      settings = this.automationSettingsRepository.create({
        institutionId,
        type: AutomationSettingsType.INVOICE_CAMPAIGN_DUPLICATION,
        settings: {
          enableInvoiceCampaignDuplication: false,
          invoiceCampaignTemplateId: null,
          invoiceCampaignDuplicationDay: 1,
        },
      })
      await this.automationSettingsRepository.save(settings)
    }
    return settings
  }
