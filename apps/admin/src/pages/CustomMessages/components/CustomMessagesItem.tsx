...
const CustomMessageItem = ({
  item,
  isWhatsappConnected: isWhatsappConnectedProp,
  isWhatsappLoading: isWhatsappLoadingProp,
  ...
  const isMetaConnected =
    signupQuery.data?.status?.toLowerCase() === 'connected' ||
    signupQuery.data?.status?.toLowerCase() === 'completed'

  const isWhatsappConnected =
    isWhatsappConnectedProp ??
    (isMetaConnected ||
      whatsappSessionStatus?.data?.status === WhatsAppConnectionStatus.READY)

  const isWhatsappLoading =
    isWhatsappLoadingProp ??
    (signupQuery.isLoading || isWhatsappSessionStatusLoading)
