...
const PageCountGraph = ({ outerChartDate }: PropTypes): JSX.Element => {
  ...
  const { data, isLoading, isError } = useFetchGoogleAnalytics({
    ...
    dataType: GoogleAnalyticsDataType.PAGE_COUNT,
  })
  useEffect(() => {
      ...
      {data && (
        <>
          <Box justify="center" direction="col">
            <Text bold>{t(`component:googleAnalytics.pageView`)}</Text>
            <ChartDatePicker
              chartDate={chartDate}
