...
export default function DraggableFileInput({
  croppable,
  label,
  onFileUpload,
  imageUrl,
  ...
  useEffect(() => {
    const readFileUrl = async (fileUrl: string) => {
      if (isS3PrivateBucket[directory as keyof typeof isS3PrivateBucket]) {
        const result = await getS3PrivateFileUrl(fileUrl)
        setPreview(result)
      } else {
        setPreview(getS3FileUrl(fileUrl))
      }
    }
